...
exports.register = (server, baseOptions, next) => {
    ...
    internals.maximumElementsInArray = baseOptions.maximumElementsInArray || 5;
    const separator = baseOptions.separator || ',';
    ...
    server.ext('onPreResponse', (request, reply) => {
        ...
        if (internals.routeMap.has(request.route.path) && (isApplicationCsvAcceptHeader || isTextCsvAcceptHeader)) {
            ...
            return reply(internals.schemaToCsv(schema, request.response.source, separator)).header('content-type', responseHeader);
        }
...
internals.parseSchema = (joiSchema, keyName, arrayFlag) => {
    ...
    return [{ [keyName]: [keyName] }]; //First square brackets are used to convert to String, second brackets are used to convert to an array
};
