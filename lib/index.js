...
internals.parseSchema = (joiSchema, dynamicSchemas, keyName, path, parentIsArray) => {
    ...
    if (joiSchema.type === 'array') {
        ...
        if (keyName && parsedItem) {
            ...
            for (let i = 1; i <= internals.maximumElementsInArray; ++i) {
                prefixedItemArray.push(parsedItem.map((headerQuery) => {

                    const key = Object.keys(headerQuery)[0];
                    let name;
                    if (label === '_') {
                        name = parsedItem.length === 1 || i === 1 ? `${key}` : `${key} ${i}`;
                    }
                    else if (label) {
                        name = parsedItem.length === 1 || i === 1 ? `${label} ${key}` : `${label} ${i} ${key}`;
                    }
                    else {
                        name = parsedItem.length === 1 ? `${keyName} ${i}` : `${keyName} ${i} ${key}`;
                    }
