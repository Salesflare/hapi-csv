    ...
    register: (server, options) => {
        ...
        server.ext('onPreResponse', async (request, h) => {
            ...
            const promises = Object.keys(dynamicHandlerObject).map((path) => {
                ...
                return handler(request).then((dynamicSchema) => {
            ...
            return internals.processSchema(schema, resolvedSchemasObject, request.response.source, options.resultKey, (headerQueryMap, dataset) => {
                ...
                /**
                 * @todo Refactor to Set Cors Headers properly.
                 */
                const cors = h.response().request.route.settings.cors;
                if (cors) {
                    if (cors.origin) {
